...
// M204 - Set default acceleration: S normal moves T filament only moves (M204 S3000 T7000) im mm/sec^2  also sets minimum segment time
// in ms (B20000) to prevent buffer underruns and M20 minimum feedrate M205 -  advanced settings:  minimum travel speed S=while printing
// T=travel only,  B=minimum segment time X= maximum xy jerk, Z=maximum Z jerk, E=maximum E jerk M206 - set additional homeing offset
// M207 - set retract length S[positive mm] F[feedrate mm/sec] Z[additional zlift/hop]
// M208 - set recover=unretract length S[positive mm surplus to the M207 S*] F[feedrate mm/sec]
// M209 - S<1=true/0=false> enable automatic retract detect if the slicer did not support G10/11: every normal extrude-only move will be
// classified as retract depending on the direction. M218 - set hotend offset (in mm): T<extruder_number> X<offset_on_X> Y<offset_on_Y>
...
inline void get_sdcard_commands() {
  if (!card.sdprinting() || card.pause() || (printing_state == PRINT_STATE_ABORT))
    return;
  ...
  while (buflen < BUFSIZE && !card_eof) {
    ...
    if (card.errorCode()) {
      ...
      card.clearError();
      // Screw it, if we are near the end of a file with an error, act if the file is finished. Hopefully preventing the hang at the
      // end.
    ...
    if (card_eof || sd_char == '\n' || sd_char == '\r' || ((sd_char == '#' || sd_char == ':') && !comment_mode)) {
      if (card_eof) {
...
#define DEFINE_PGM_READ_ANY(type, reader) \
  static inline type pgm_read_any(const type *p) { return pgm_read_##reader##_near(p); }
...
#define XYZ_CONSTS_FROM_CONFIG(type, array, CONFIG)                                \
  ...
  static inline type array(int axis) { return pgm_read_any(&array##_P[axis]); }
    ...
    switch ((int)code_value()) {
      ...
      case 10:  // G10 retract
        ...
        if (!retracted) {
          ...
          retract_recover_length = current_position[E_AXIS] -
                                   destination[E_AXIS];  // Set the recover length to whatever distance we retracted so we recover properly.
          retracted = true;
          prepare_move(strCmd);
        ...
        while (target_direction ? (isHeatingHotend(tmp_extruder)) : (isCoolingHotend(tmp_extruder) && (CooldownNoWait == false))) {
          ...
          if ((residencyStart == -1 && target_direction && (degHotend(tmp_extruder) >= (degTargetHotend(tmp_extruder) - TEMP_WINDOW))) ||
              ...
              (residencyStart > -1 && labs(degHotend(tmp_extruder) - degTargetHotend(tmp_extruder)) > TEMP_HYSTERESIS &&
               (!target_direction || !CooldownNoWait))) {
...
// PWM for HEATER_2_PIN
      ...
      case 84:  // M84
        ...
        if (code_seen(strCmd, 'S')) {
          ...
          bool all_axis = !((code_seen(strCmd, axis_codes[X_AXIS])) || (code_seen(strCmd, axis_codes[Y_AXIS])) || (code_seen(strCmd, axis_codes[Z_AXIS])) ||
                            (code_seen(strCmd, axis_codes[E_AXIS])));
          if (all_axis) {
            finishAndDisableSteppers();
          }
          else {
    ...
    case 202: // M202
      ...
      case 205:  // M205 advanced settings:  minimum travel speed S=while printing T=travel only,  B=minimum segment time X= maximum xy
                 // jerk, Z=maximum Z jerk
      {
        if (code_seen(strCmd, 'S'))
      ...
      case 209:  // M209 - S<1=true/0=false> enable automatic retract detect if the slicer did not support G10/11: every normal
      ...
      {
        if (code_seen(strCmd, 'S')) {
          // fix potential -1 value from Cura's post g process script
          int _feedmultiply = code_value();
          if (_feedmultiply < 0) {
            feedmultiply = _saved_feedmultiply;
          }
          else {
            _saved_feedmultiply = feedmultiply;
            feedmultiply = _feedmultiply;
          }
        }
      } break;
      ...
      case 300:  // M300
      {
        ...
        if (beepS > 0) {
          ...
          if (beepP > 4000)
            beepP = 4000;  // prevent watchdog from tripping
          uint32_t loops = ((((uint32_t)beepP) * 500) / notch);
          for (uint32_t _i = 0; _i < loops; _i++) {
            WRITE(BEEPER, HIGH);
            delayMicroseconds(notch);
            WRITE(BEEPER, LOW);
            delayMicroseconds(notch);
          }
      ...
      case 601:  // M601 Pause in UltiLCD2, X[pos] Y[pos] Z[relative lift] L[later retract distance]
      {
        ...
        if ((printing_state != PRINT_STATE_ABORT) && (card.sdprinting() || HAS_SERIAL_CMD)) {
          ...
          if (bAddRetract) {
            ...
            target[E_AXIS] += addRetractLength;
            plan_buffer_line(target[X_AXIS], target[Y_AXIS], target[Z_AXIS], target[E_AXIS], retract_feedrate / 60,
          ...
          // move z back
          plan_buffer_line(
            current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], target[E_AXIS], homing_feedrate[Z_AXIS] / 60, active_extruder);
      ...
      case 10001:  // M10001 - Draw text on LCD, M10002 X0 Y0 SText (when X is left out, it will draw centered)
      {
        ...
        if (code_seen(strCmd, 'X')) {
          ...
          if (code_seen(strCmd, 'Y'))
            y = code_value_long();
          if (code_seen(strCmd, 'S'))
            lcd_lib_draw_string(x, y, strchr_pointer + 1);
        }
        else {
          if (code_seen(strCmd, 'Y'))
            y = code_value_long();
      ...
      case 10002:  // M10002 - Draw inverted text on LCD, M10002 X0 Y0 SText (when X is left out, it will draw centered)
      {
        ...
        if (code_seen(strCmd, 'X')) {
          ...
          if (code_seen(strCmd, 'Y'))
            y = code_value_long();
          if (code_seen(strCmd, 'S'))
            lcd_lib_clear_string(x, y, strchr_pointer + 1);
        }
        else {
          if (code_seen(strCmd, 'Y'))
            y = code_value_long();
      ...
      case 10003:  // M10003 - Draw square on LCD, M10003 X1 Y1 W10 H10
      {
        ...
        if (code_seen(strCmd, 'Y'))
          y = code_value_long();
        if (code_seen(strCmd, 'W'))
          w = code_value_long();
        if (code_seen(strCmd, 'H'))
          h = code_value_long();
...
void calculate_delta(float cartesian[3]) {
  delta[X_AXIS] = sqrt(sq(DELTA_DIAGONAL_ROD) - sq(DELTA_TOWER1_X - cartesian[X_AXIS]) - sq(DELTA_TOWER1_Y - cartesian[Y_AXIS])) + cartesian[Z_AXIS];
  delta[Y_AXIS] = sqrt(sq(DELTA_DIAGONAL_ROD) - sq(DELTA_TOWER2_X - cartesian[X_AXIS]) - sq(DELTA_TOWER2_Y - cartesian[Y_AXIS])) + cartesian[Z_AXIS];
  delta[Z_AXIS] = sqrt(sq(DELTA_DIAGONAL_ROD) - sq(DELTA_TOWER3_X - cartesian[X_AXIS]) - sq(DELTA_TOWER3_Y - cartesian[Y_AXIS])) + cartesian[Z_AXIS];
  /*
...
static void prepare_move(const char *cmd) {
  ...
  if (card.sdprinting() && (printing_state == PRINT_STATE_RECOVER) && (destination[Z_AXIS] >= recover_height - 0.01f)) {
    ...
    if ((current_position[X_AXIS] == destination[X_AXIS]) && (current_position[Y_AXIS] == destination[Y_AXIS])) {
      ...
      plan_buffer_line(
        destination[X_AXIS], destination[Y_AXIS], destination[Z_AXIS], destination[E_AXIS], feedrate * feedmultiply / 60 / 100.0, active_extruder);
    }
  }
#endif
...
void controllerFan() {
  if ((millis() - lastMotorCheck) >= 2500)  // Not a time critical function, so we only check every 2500ms
  {
    ...
    if ((millis() - lastMotor) >= (CONTROLLERFAN_SECS * 1000UL) ||
        lastMotor == 0)  // If the last time any driver was enabled, is longer since than CONTROLLERSEC...
    {
  ...
  if ((m - previous_millis_cmd) > EXTRUDER_RUNOUT_SECONDS * 1000)
    if (degHotend(active_extruder) > EXTRUDER_RUNOUT_MINTEMP) {
      ...
      plan_buffer_line(current_position[X_AXIS],
                       ...
                       EXTRUDER_RUNOUT_SPEED / 60. * EXTRUDER_RUNOUT_ESTEPS / e_steps_per_unit(active_extruder),
                       active_extruder);
