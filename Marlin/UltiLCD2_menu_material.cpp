...
#define NOZZLE_INDEX (cache._byte[0])
#define MATERIAL_LOAD_SUCCESSFUL (cache._bool[1])
...
void lcd_menu_change_material_preheat()
{
    ...
    if (temp > target - 5 && temp < target + 5)
    {
      ...
      plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS], retract_feedrate, menu_extruder);
...
static void lcd_menu_material_import_done()
{
    ...
    {
        lcd_lib_draw_string_centerP(20, PSTR("Material import"));
        lcd_lib_draw_string_centerP(30, PSTR("successful"));
    }
    else
    {
        lcd_lib_draw_string_centerP(20, PSTR("Error during"));
        lcd_lib_draw_string_centerP(30, PSTR("material import!"));
    }
    lcd_lib_update_screen();
}
...
static void lcd_retraction_item_per_nozzle(uint8_t nr, uint8_t offsetY, uint8_t flags)
{
    char buffer[LINE_ENTRY_TEXT_LENGTH+1] = {0};
    if (nr == 0)
    {
        lcd_cpyreturn(buffer);
    }
...
static void lcd_retraction_details_per_nozzle(uint8_t nr)
{
    ...
    if (nr == 0)
        ...
        float_to_string2(material[menu_extruder].retraction_length[NOZZLE_INDEX], buffer, PSTR("mm"));
    else if(nr == 2)
        int_to_string(material[menu_extruder].retraction_speed[NOZZLE_INDEX] / 60 + 0.5, buffer, PSTR("mm/sec"));
    lcd_lib_draw_string_left(BOTTOM_MENU_YPOS, buffer);
}
...
static void lcd_menu_material_retraction_settings_per_nozzle()
{
    ...
    if (lcd_lib_button_pressed)
    {
        if (IS_SELECTED_SCROLL(0))
            ...
            LCD_EDIT_SETTING_FLOAT001(material[menu_extruder].retraction_length[NOZZLE_INDEX], "Retract length", "mm", 0, 50);
        else if (IS_SELECTED_SCROLL(2))
            LCD_EDIT_SETTING_SPEED(material[menu_extruder].retraction_speed[NOZZLE_INDEX], "Retract speed", "mm/sec", 0, max_feedrate[E_AXIS] * 60);
    }
    lcd_lib_update_screen();
}

static void lcd_material_retraction_settings_callback(uint8_t nr, uint8_t offsetY, uint8_t flags)
{
    char buffer[LINE_ENTRY_TEXT_LENGTH+1] = {0};
    if (nr == 0)
    {
...
static void lcd_material_settings_retraction_details_callback(uint8_t nr)
{
    ...
    char* c = float_to_string2(material[menu_extruder].retraction_length[nr - 1], buffer, PSTR("mm "));
    int_to_string(material[menu_extruder].retraction_speed[nr - 1] / 60.0 + 0.5, c, PSTR("mm/s"));
    lcd_lib_draw_string_left(BOTTOM_MENU_YPOS, buffer);
}
...
static void lcd_menu_material_retraction_settings()
{
    ...
    if (lcd_lib_button_pressed)
    {
        ...
        {
            NOZZLE_INDEX = SELECTED_SCROLL_MENU_ITEM() - 1;
            menu.add_menu(menu_t(lcd_menu_material_retraction_settings_per_nozzle));
        }
    }
    lcd_lib_update_screen();
}
...
static void lcd_material_settings_details_callback(uint8_t nr)
{
    ...
    if (nr == 0)
    {
        strlcpy(buffer, material[menu_extruder].name, sizeof(buffer));
    }
    else if (nr == 1)
    {
        char* c = buffer;
...
static void lcd_menu_material_settings()
{
    lcd_scroll_menu(PSTR("MATERIAL"), 7 + BED_MENU_OFFSET, lcd_material_settings_callback, lcd_material_settings_details_callback);
    if (lcd_lib_button_pressed)
    {
        if (IS_SELECTED_SCROLL(0))
        {
        ...
        }else if (IS_SELECTED_SCROLL(1))
        {
            ...
            menu.add_menu(menu_t(lcd_menu_material_retraction_settings));
...
void lcd_material_store_material(uint8_t nr)
{
    ...
    for(uint8_t n=0; n<MATERIAL_NOZZLE_COUNT; ++n)
    {
        ...
        eeprom_write_word(EEPROM_MATERIAL_EXTRA_RETRACTION_LENGTH_OFFSET(nr, n), material[menu_extruder].retraction_length[n] * EEPROM_RETRACTION_LENGTH_SCALE);
        eeprom_write_byte(EEPROM_MATERIAL_EXTRA_RETRACTION_SPEED_OFFSET(nr, n), material[menu_extruder].retraction_speed[n] / 60.0 * EEPROM_RETRACTION_SPEED_SCALE);
    }
}
