...
static void lcd_advanced_details(uint8_t nr) {
...
#endif
  {
    ...
    int_to_string(
      ...
      int_to_string(
        int(dsp_temperature[1]), int_to_string(int(target_temperature[0]), int_to_string(int(dsp_temperature[0]), buffer, PSTR("C/")), PSTR("C ")), PSTR("C/")),
      PSTR("C"));
#else
...
void homeHead() { enquecommand_P(PSTR("G28 X0 Y0")); }

void homeBed() { enquecommand_P(PSTR("G28 Z0")); }

void homeAll() { enquecommand_P(PSTR("G28")); }
...
FORCE_INLINE static void lcd_dual_nozzle_heatup() { lcd_select_nozzle(start_nozzle_heatup, NULL); }

FORCE_INLINE static void lcd_dual_insert_material() { lcd_select_nozzle(start_insert_material, NULL); }

void lcd_dual_move_material() { lcd_select_nozzle(start_move_material, NULL); }
...
static void lcd_menu_maintenance_advanced_heatup() {
  if (lcd_lib_encoder_pos / ENCODER_TICKS_PER_SCROLL_MENU_ITEM != 0) {
    target_temperature[active_extruder] = constrain(
      int(target_temperature[active_extruder]) + (lcd_lib_encoder_pos / ENCODER_TICKS_PER_SCROLL_MENU_ITEM), 0, get_maxtemp(active_extruder) - 15);
...
void lcd_menu_maintenance_advanced_bed_heatup() {
  if (lcd_lib_encoder_pos / ENCODER_TICKS_PER_SCROLL_MENU_ITEM != 0) {
    target_temperature_bed = constrain(int(target_temperature_bed) + (lcd_lib_encoder_pos / ENCODER_TICKS_PER_SCROLL_MENU_ITEM), 0, BED_MAXTEMP - 15);
    lcd_lib_encoder_pos = 0;
  }
  if (lcd_lib_button_pressed)
    lcd_change_to_previous_menu();
...
static void lcd_menu_screen_contrast() {
  ...
  lcd_lib_draw_bargraph(
    LCD_CHAR_MARGIN_LEFT + 2 * LCD_CHAR_SPACING, 34, LCD_GFX_WIDTH - LCD_CHAR_MARGIN_RIGHT - 2 * LCD_CHAR_SPACING, 40, float(lcd_contrast) / 255);
