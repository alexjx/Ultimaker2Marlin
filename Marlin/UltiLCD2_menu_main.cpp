...
static void lcd_menu_breakout() {
  ...
  if (lcd_lib_encoder_pos * 2 > 128 - BREAKOUT_PADDLE_WIDTH - 1)
    lcd_lib_encoder_pos = (128 - BREAKOUT_PADDLE_WIDTH - 1) / 2;
  ball_x += ball_dx;
  ball_y += ball_dy;
  if (ball_x < 1 << 8)
    ball_dx = abs(ball_dx);
...
FORCE_INLINE static void lcd_dual_material_main() { lcd_select_nozzle(start_material_main, NULL); }

void lcd_dual_material_settings() { lcd_select_nozzle(start_material_settings, NULL); }

void lcd_dual_material_change() { lcd_select_nozzle(start_material_change, NULL); }
#endif
...
FORCE_INLINE static void lcd_toggle_preheat_nozzle0() { lcd_toggle_preheat_nozzle(0); }

FORCE_INLINE static void lcd_preheat_tune_nozzle0() { lcd_preheat_tune_nozzle(0); }

#if EXTRUDERS > 1
FORCE_INLINE static void lcd_toggle_preheat_nozzle1() { lcd_toggle_preheat_nozzle(1); }

FORCE_INLINE static void lcd_preheat_tune_nozzle1() { lcd_preheat_tune_nozzle(1); }
#endif
...
static void drawPreheatSubmenu(uint8_t nr, uint8_t &flags) {
  ...
  if (nr == index++) {
    LCDMenu::drawMenuBox(LCD_CHAR_MARGIN_LEFT * 2, BOTTOM_MENU_YPOS, 35, LCD_CHAR_HEIGHT, flags);
    ...
    LCDMenu::drawMenuBox(49, BOTTOM_MENU_YPOS, 35, LCD_CHAR_HEIGHT, flags);
    if (flags & MENU_SELECTED) {
    ...
    LCDMenu::drawMenuBox(89, BOTTOM_MENU_YPOS, 35, LCD_CHAR_HEIGHT, flags);
    if (flags & MENU_SELECTED) {
    ...
    LCDMenu::drawMenuBox(
      LCD_CHAR_MARGIN_LEFT, 47 - (EXTRUDERS * LCD_LINE_HEIGHT) - (BED_MENU_OFFSET * LCD_LINE_HEIGHT), 3 * LCD_CHAR_SPACING, LCD_CHAR_HEIGHT, flags);
    if (flags & MENU_SELECTED) {
      lcd_lib_clear_gfx(LCD_CHAR_MARGIN_LEFT + LCD_CHAR_SPACING,
                        47 - (EXTRUDERS * LCD_LINE_HEIGHT) - (BED_MENU_OFFSET * LCD_LINE_HEIGHT),
                        PREHEAT_FLAG(0) ? checkbox_on : checkbox_off);
    }
    else {
      lcd_lib_draw_gfx(LCD_CHAR_MARGIN_LEFT + LCD_CHAR_SPACING,
                       47 - (EXTRUDERS * LCD_LINE_HEIGHT) - (BED_MENU_OFFSET * LCD_LINE_HEIGHT),
                       PREHEAT_FLAG(0) ? checkbox_on : checkbox_off);
    }
  }
    ...
    LCDMenu::drawMenuBox(LCD_CHAR_MARGIN_LEFT, 39 - (BED_MENU_OFFSET * LCD_LINE_HEIGHT), 3 * LCD_CHAR_SPACING, LCD_CHAR_HEIGHT, flags);
    if (flags & MENU_SELECTED) {
    ...
    LCDMenu::drawMenuBox(LCD_CHAR_MARGIN_LEFT, 40, 3 * LCD_CHAR_SPACING, LCD_CHAR_HEIGHT, flags);
    if (flags & MENU_SELECTED) {
    ...
    LCDMenu::drawMenuString(LCD_GFX_WIDTH - LCD_CHAR_MARGIN_RIGHT - 4 * LCD_CHAR_SPACING, 40, 24, LCD_CHAR_HEIGHT, buffer, ALIGN_RIGHT | ALIGN_VCENTER, flags);
  }
#endif
}
...
static void lcd_main_preheat() {
  ...
  // lcd_lib_draw_heater(LCD_GFX_WIDTH-LCD_CHAR_MARGIN_RIGHT-70, ypos,
  // getHeaterPower(-1));
...
void drawMainStandard(uint8_t nr, uint8_t &flags) {
  uint8_t menu_index = 0;
  if (nr == menu_index++) {
    LCDMenu::drawMenuString_P(LCD_CHAR_MARGIN_LEFT + 3, LCD_LINE_HEIGHT, 52, LCD_LINE_HEIGHT * 4, PSTR("PRINT"), ALIGN_CENTER, flags);
  }
  else if (nr == menu_index++) {
    LCDMenu::drawMenuString_P(LCD_GFX_WIDTH / 2 + LCD_CHAR_MARGIN_LEFT + 3, LCD_LINE_HEIGHT, 52, LCD_LINE_HEIGHT * 4, PSTR("MATERIAL"), ALIGN_CENTER, flags);
  }
  else if (nr == menu_index++) {
    LCDMenu::drawMenuString_P(
      LCD_GFX_WIDTH / 2 - 7 * LCD_CHAR_SPACING, BOTTOM_MENU_YPOS, 14 * LCD_CHAR_SPACING, LCD_CHAR_HEIGHT, PSTR("MAINTENANCE"), ALIGN_CENTER, flags);
  }
}
...
void drawMainExpert(uint8_t nr, uint8_t &flags) {
  uint8_t menu_index = 0;
  if (nr == menu_index++) {
    LCDMenu::drawMenuString_P(LCD_CHAR_MARGIN_LEFT + 2, 17, 52, 13, PSTR("PREHEAT"), ALIGN_LEFT | ALIGN_VCENTER, flags);
  }
  else if (nr == menu_index++) {
    LCDMenu::drawMenuString_P(LCD_CHAR_MARGIN_LEFT + 2, 31, 52, 13, PSTR("PRINT"), ALIGN_LEFT | ALIGN_VCENTER, flags);
  }
  else if (nr == menu_index++) {
    LCDMenu::drawMenuString_P(LCD_CHAR_MARGIN_LEFT + 2, 45, 52, 13, PSTR("ADVANCED"), ALIGN_LEFT | ALIGN_VCENTER, flags);
  }
  else if (nr == menu_index++) {
    LCDMenu::drawMenuString_P(LCD_GFX_WIDTH / 2 + LCD_CHAR_MARGIN_LEFT + 3, 17, 52, 13, PSTR("SETTINGS"), ALIGN_LEFT | ALIGN_VCENTER, flags);
  }
  else if (nr == menu_index++) {
    LCDMenu::drawMenuString_P(LCD_GFX_WIDTH / 2 + LCD_CHAR_MARGIN_LEFT + 3, 31, 52, 13, PSTR("MOVE"), ALIGN_LEFT | ALIGN_VCENTER, flags);
  }
  else if (nr == menu_index++) {
    LCDMenu::drawMenuString_P(LCD_GFX_WIDTH / 2 + LCD_CHAR_MARGIN_LEFT + 3, 45, 52, 13, PSTR("CHANGE"), ALIGN_LEFT | ALIGN_VCENTER, flags);
  }
}
